...
uint8_t flash_backup_to_app(int32_t app_size)
{
  uint8_t flash_state = flash_erase(ApplicationAddress, app_size);
  uint32_t flash_dest = ApplicationAddress;
  uint32_t backup_source = BackAppAddress;
  ...
  for(uint32_t pointer = 0; 
  (pointer < app_size) && (flash_dest < ApplicationAddress + app_size); 
  pointer += 4)
  {
    flash_write(flash_dest, *(uint32_t*)backup_source);

    if(*(uint32_t*)flash_dest != *(uint32_t*)backup_source)
    {
      return 1;
    }
